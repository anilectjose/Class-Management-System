...
$su_id=$_GET['s_id'];
include 'db2.php';
mysqli_query($con,"SELECT * FROM `student_db` join role_db on role_db.role_id=student_db.role_id where student_db.role_id='$su_id'");
if(isset($_POST['name']))
{
	@$name = $_POST['name'];
	@$email = $_POST['email'];
	...
	mysqli_query($con, "UPDATE `student_db` SET `name`='$name',`phone`='$mobile',`email`='$email',`gender`='$gende',`address`='$adress',`year`='$yr',`image`='' where role_id='$su_id'");

    mysqli_query($con, "UPDATE `role_db` SET `username`='$username',`password`='$password' WHERE role_id='$su_id'");
...
?>
   ...
   ?>
    ...
    <main class="app-content">
      <div class="app-title">
        <div>
          <h1><i class="fa fa-dashboard"></i> Edit Students</h1>
          <p></p>
        </div>
